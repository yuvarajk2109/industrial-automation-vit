"""
CaneNexus – Steel Defect Detection Inference
Runs the DDA-ViT model on a steel strip image and returns structured results.
Ported from Code/DDA-ViT/DDA-ViT.py predict_steel().
"""
...
def predict_steel(image_path: str) -> dict:
    ...
    Returns:
        Structured result dict with defect summary, mask paths, and timing.
    """
    model = get_model()
    device = get_device()
    ...
    for i, cls_name in enumerate(STEEL_CLASSES):
        cls_idx = i + 1  # classes are 1-indexed in steel
        area_pct = 0.0
        detected = False
    ...
    # Dominant defect (class with highest area, excluding background)
    defect_classes_only = {
