"""
CaneNexus – Sugar Crystallisation Knowledge Graph
State → Condition → Action graph for sugar quality control.
Ported from Code/DDA-ViT/sugar.py (lines 97-449).
"""
...
def infer_supersaturation_range(crystal_state: str, confidence: float) -> tuple:
    """Infer supersaturation range based on state and prediction confidence."""
...
def build_inferred_state(vit_output: dict) -> dict:
    """
    Build an inferred process state from the ViT classification output.

    Args:
        vit_output: {"crystal_state": str, "confidence": float}

    Returns:
        Enriched state dict with nucleation risk, growth stability, etc.
    """
...
def build_sugar_kg() -> nx.DiGraph:
    ...
    KG.add_edge("METASTABLE", "INTERMEDIATE",
                edge_type="state_transition", trigger="excess_evaporation")
    KG.add_edge("INTERMEDIATE", "LABILE",
                edge_type="state_transition", trigger="runaway_supersaturation")
    KG.add_edge("LABILE", "INTERMEDIATE",
                edge_type="state_transition", trigger="cooling_or_dilution")
...
def _infer_actions(graph: nx.DiGraph, active_state: str) -> list:
    """Traverse the KG from a state node to find all triggered actions."""
    actions = set()
...
def _score_transitions(graph: nx.DiGraph, active_state: str) -> dict:
    """Score possible state transitions based on risk and controllability."""
    scores = {}
...
def evaluate_sugar_kg(prediction: dict) -> dict:
    """
    Given a sugar classification prediction, traverse the KG
    and determine recommended actions and state info.
